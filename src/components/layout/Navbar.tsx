    ...
    <header className="fixed top-0 inset-x-0 z-50 glass border-b border-border">
      <nav className="flex justify-between items-center max-w-5xl mx-auto px-6 h-14">
        ...
        <div className="hidden sm:flex items-center gap-8">
          {links.map((l) => (
            <button key={l.id} onClick={() => scrollTo(l.id)} className="text-[12px] text-text-secondary hover:text-text transition-colors font-mono uppercase tracking-wider">
              {l.label}
            </button>
          ))}
        </div>
        <button onClick={() => scrollTo('comprar')} className="relative p-2 -mr-2 rounded-full hover:bg-white/5 transition-colors">
